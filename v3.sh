...
#
# Requirements:
#   - Linux with systemd or SysVinit
#   - Root privileges for installation
#   - curl, tar
#   - x86_64 or arm64 architecture
#
# Author: Troray (original), adapted for SysVinit by Assistant
...
check_systemd_support() {
    if command -v systemctl >/dev/null 2>&1; then
        ...
        echo -e "${YELLOW_COLOR}警告：当前环境不支持 systemd，将使用 SysVinit 模式管理服务${RES}"
    fi
}
...
GET_INSTALLED_PATH() {
    # 从 service 文件中获取工作目录 (systemd)
    if [ "$SYSTEMD_SUPPORTED" = true ] && [ -f "/etc/systemd/system/alist.service" ]; then
    ...
    # 从 SysVinit 脚本中获取路径
    if [ "$SYSTEMD_SUPPORTED" = false ] && [ -f "/etc/init.d/alist" ]; then
        installed_path=$(grep "DAEMON=" /etc/init.d/alist | cut -d'=' -f2 | sed 's/\/alist$//')
        if [ -f "$installed_path/alist" ]; then
            echo "$installed_path"
            return 0
        fi
...
EOF
    ...
    # SysVinit 模式，创建 /etc/init.d/alist 脚本
...
EOF
    ...
    # 启用服务（如果支持 update-rc.d）
    if command -v update-rc.d >/dev/null 2>&1; then
        update-rc.d alist defaults >/dev/null 2>&1
        echo -e "${GREEN_COLOR}Alist 服务已注册到 SysVinit${RES}"
    else
        echo -e "${YELLOW_COLOR}警告：无法自动启用服务，请手动运行 /etc/init.d/alist start${RES}"
    fi
  fi
}
  ...
  case "$choice" in
    ...
    4)
      ...
      if [ "$SYSTEMD_SUPPORTED" = true ]; then
        ...
        if ps aux | grep -v grep | grep "$INSTALL_PATH/alist" >/dev/null; then
          echo -e "${GREEN_COLOR}Alist 当前状态为：运行中${RES}"
        else
          echo -e "${RED_COLOR}Alist 当前状态为：停止${RES}"
        fi
