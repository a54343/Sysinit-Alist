...
Start_aria2() {
    ...
    if [[ -x /etc/init.d/aria2 ]]; then
        /etc/init.d/aria2 start
    elif command -v service >/dev/null 2>&1; then
        service aria2 start
    else
        echo -e "${Error} 无法启动 Aria2，服务脚本未找到或不支持 service 命令 !" && exit 1
    fi
    check_pid
    [[ ! -z ${PID} ]] && echo -e "${Info} Aria2 启动成功 !" || echo -e "${Error} Aria2 启动失败 !"
}
...
Stop_aria2() {
    ...
    if [[ -x /etc/init.d/aria2 ]]; then
        ...
        echo -e "${Error} 无法停止 Aria2，服务脚本未找到或不支持 service 命令 !" && exit 1
    fi
    check_pid
    [[ -z ${PID} ]] && echo -e "${Info} Aria2 停止成功 !" || echo -e "${Error} Aria2 停止失败 !"
}
...
Restart_aria2() {
    ...
    if [[ -x /etc/init.d/aria2 ]]; then
        ...
        echo -e "${Error} 无法重启 Aria2，服务脚本未找到或不支持 service 命令 !" && exit 1
    fi
    check_pid
    [[ ! -z ${PID} ]] && echo -e "${Info} Aria2 重启成功 !" || echo -e "${Error} Aria2 重启失败 !"
}
